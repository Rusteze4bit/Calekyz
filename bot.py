...
def fetch_and_analyze():
    ...
    global signal_msg_id, last_expired_id, last_prep_id
    ...
    if best_market:
        ...
        signal_msg_id = send_telegram_message(main_msg)
        ...
        # --- Delete signal + expiration 30s after prep ---
        time.sleep(30)
        delete_message(signal_msg_id)
        delete_message(last_expired_id)
...
def schedule_signals():
    while True:
        fetch_and_analyze()
        # 10 min cycle: 2 min signal + 1 min wait + 30s delete = ~3.5 mins
        time.sleep(600 - 210)
